 ...
 */
function getCatInfoTest(type, catId, customCats = []) {
...
describe('Transaction structure', () => {
  ...
  it('has all required fields', () => {
    expect(validTransaction).toHaveProperty('__backendId');
  ...
  it('has valid type', () => {
    expect(['expense', 'income']).toContain(validTransaction.type);
  });
  ...
  it('stores in localStorage correctly', () => {
    ...
    lsSet(SK.TX, transactions);

    const retrieved = lsGet(SK.TX, []);
    expect(retrieved.length).toBe(1);
    expect(retrieved[0].amount).toBe(100);
    expect(retrieved[0].__backendId).toBe('tx_123');
  });
});
...
describe('Budget allocation structure', () => {
  ...
  it('stores monthly allocations by category', () => {
    ...
    const retrieved = lsGet(SK.ALLOC, {});
...
describe('Savings goals structure', () => {
  ...
  it('stores savings goals with progress', () => {
    ...
    const retrieved = lsGet(SK.SAVINGS, {});
